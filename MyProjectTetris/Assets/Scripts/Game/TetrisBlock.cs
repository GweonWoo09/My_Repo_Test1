...
public class TetrisBlock : MonoBehaviour
{
    ...
    private void Start()
    {
        spawnBlock = GetComponent<SpawnBlocks>();
    ...
    void RotateBlock()
    {
        ...
        if (tag is "Mino_I" or "Mino_S" or "Mino_Z") // 도는 범위가 넓어서 다시 잡아줌. 90도로 돌아갔다가 다시 원래 위치로
        {
            if (transform.localEulerAngles.z < 0 || transform.localEulerAngles.z > 90)
                transform.localEulerAngles = Vector3.zero;
        }
        else if (tag == "Mino_O") // 돌릴필요 없음
            transform.localEulerAngles = Vector3.zero;
    ...
    void CheckForLines()
    {
        ...
        if (cnt == 0)
        {
            UI.scoreUI.text = $"Score : {UI.score += 10}";
        }
        else
        {
            Debug.Log("deleted lines : " + cnt);
            // Score
            UI.scoreUI.text = $"Score : {UI.score += cnt * 100}";
        }
    }
